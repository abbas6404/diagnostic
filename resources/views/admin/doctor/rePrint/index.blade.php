                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" autocomplete="off" class="form-control form-control-sm" id="invoice_search" placeholder="Enter invoice number, patient name, or phone..." tabindex="1">
                                        </div>
                                    </div>
                                    <div class="row mb-2">
                                ...
                                <table class="table table-sm table-bordered mb-0" id="consultantTicketsTable">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th>Doctor Name</th>
                                            <th style="width: 80px;" class="text-end">Fee</th>
...
<script>
    ...
    function searchInvoices(query) {
        fetch(`/admin/doctor/reprint/search?query=${encodeURIComponent(query)}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
    ...
    function selectInvoiceFromSearch(invoiceId) {
        ...
        fetch(`/admin/doctor/reprint/invoice/${numericInvoiceId}/details`)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
            ...
            .then(data => {
                ...
                updateConsultantTicketsTable(data.consultant_tickets);
    ...
    function updateConsultantTicketsTable(consultantTickets) {
        this.consultantTickets = consultantTickets || [];
        const tbody = document.querySelector('#consultantTicketsTable tbody');
        tbody.innerHTML = '';
        
        if (consultantTickets && consultantTickets.length > 0) {
            consultantTickets.forEach(ticket => {
                const row = document.createElement('tr');
                row.className = 'item-print-row';
                row.dataset.itemId = ticket.id;
                ...
                if (ticket.ticket_status === 'completed') {
                    ...
                    statusBadge = '<span class="badge bg-secondary">' + (ticket.ticket_status || 'Unknown') + '</span>';
                }
    ...
    function printInvoice(printType, copies) {
        fetch('/admin/doctor/reprint/print', {
            method: 'POST',
            headers: {
