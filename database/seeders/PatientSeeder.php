<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class PatientSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        DB::table('patients')->insert([
            'id' => 1,
            'patient_id' => 'P250722001',
            'reg_date' => '2025-01-01',
            'name' => 'Ariyan Abdullah',
            'father_husband_name' => 'Ariyan Abdullah', 
            'address' => 'Dhaka', 
            'phone' => '01717171717',   
            'email' => 'ariyan@gmail.com',
            'dob' => '1990-01-01',
            'gender' => 'Male',
            'blood_group' => 'A+',
            'religion' => 'Islam',
            'occupation' => 'Student',
            'reg_fee' => '100',
            'nationality' => 'Bangladeshi',
            'patient_type' => 'Inpatient',
            'created_by' => '1',
            'updated_by' => '1',
        ]);
        DB::table('patients')->insert([
            'id' => 2,
            'patient_id' => 'P250722002',
            'reg_date' => '2025-01-01',
            'name' => 'Rakibul Islam',
            'dob' => '1993-01-01',
            'gender' => 'Male',
            'blood_group' => 'A+',
            'religion' => 'Islam',
            'occupation' => 'Student',
            'reg_fee' => '100',
            'nationality' => 'Bangladeshi',
            'phone' => '01717171718',
            'address' => 'Barishal', 
            'father_husband_name' => 'Rakibul Islam',
        ]);
        DB::table('patients')->insert([
            'id' => 3,
            'patient_id' => 'P250722003',
            'reg_date' => '2025-01-01',
            'name' => 'Arafat Hossain',
            'dob' => '1994-01-01',
            'gender' => 'Male',
            'blood_group' => 'A+',
            'religion' => 'Islam',
            'occupation' => 'Student',
            'reg_fee' => '100',
            'nationality' => 'Bangladeshi',
            'phone' => '01717171719',
            'address' => 'Sylhet', 
            'father_husband_name' => 'Arafat Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 4,
            'patient_id' => 'P250722004',
            'reg_date' => '2025-01-01', 
            'name' => 'Abdur Rahim',
            'dob' => '1995-01-01',
            'phone' => '01717171720',
            'address' => 'Rajshahi', 
            'father_husband_name' => 'Abdur Rahim',
        ]);
        DB::table('patients')->insert([ 
            'id' => 5,
            'patient_id' => 'P250722005',
            'reg_date' => '2025-01-01',
            'name' => 'Abbas Hossain',
            'dob' => '1996-01-01',
            'phone' => '01717171721',
            'address' => 'Khulna', 
            'father_husband_name' => 'Abbas Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 6,
            'patient_id' => 'P250722006',
            'reg_date' => '2025-01-01',
            'name' => 'Sojol Hossain',
            'dob' => '1997-01-01',
            'phone' => '01717171722',
            'address' => 'Chittagong', 
            'father_husband_name' => 'Sojol Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 7,
            'patient_id' => 'P240522001',
            'reg_date' => '2025-01-01',
            'name' => 'Raju Hossain',
            'gender' => 'Male',
            'dob' => '1998-01-01',
            'phone' => '01717171723',
            'address' => 'Mymensingh', 
            'father_husband_name' => 'Raju Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 8,
            'patient_id' => 'P230722001',
            'reg_date' => '2025-01-01',
            'dob' => '1999-01-01',
            'name' => 'Raju Hossain',
            'phone' => '01717171725',
            'address' => 'Rangpur', 
            'father_husband_name' => 'Raju Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 9,
            'patient_id' => 'P220722001',
            'reg_date' => '2025-01-01', 
            'name' => 'Rajaul Hossain',
            'dob' => '2000-01-01',
            'phone' => '01717171726',
            'address' => 'Barisal', 
            'father_husband_name' => 'Raju Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 10,
            'patient_id' => 'P210722089',
            'reg_date' => '2025-01-01',
            'name' => 'Rajaul Hossain',
            'dob' => '2001-01-01',
            'phone' => '01717171726',
            'address' => 'Barisal', 
            'father_husband_name' => 'Raju Hossain',
        ]);
        DB::table('patients')->insert([
            'id' => 11,
            'patient_id' => 'P230722005',
            'dob' => '2002-01-01',
            'reg_date' => '2025-01-01',
            'name' => 'Rajaul Hossain',
            'phone' => '01717171726',
            'address' => 'Barisal', 
            'father_husband_name' => 'Raju Hossain',
        ]);

    }
}
