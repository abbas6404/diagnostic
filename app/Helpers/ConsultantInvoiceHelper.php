...
class ConsultantInvoiceHelper
{
    /**
     * Generate consultant invoice number with consistent format
     * Uses the format setting from database
     */
    public static function generateConsultantInvoiceNumber()
    {
        $prefix = SystemSetting::getValue('consultant_invoice_prefix', 'CON');
        $startNumber = (int) SystemSetting::getValue('consultant_invoice_start', '1');
        $format = SystemSetting::getValue('consultant_invoice_format', 'prefix-yymmdd-number');
        $today = Carbon::now();
